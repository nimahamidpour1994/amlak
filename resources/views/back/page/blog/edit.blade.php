...
@section('content')
    <section class="direction-ltr bg-white pt-0" id="welcome">
        <div class="container-fluid my-5">
            ...
            <form action="{{route('admin.blog.update', $response['blog']->id)}}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="align-right mr-2">
                        <div class="image-upload">
                            <label for="photo">
                                ...
                                <img src="{{ $response['blog']->thumbnail!='' ? url( $response['blog']->thumbnail) : url('/front/img/structure/image-add-button.png')}}" id="slideshowfileimg"
                                     class="img-thumbnail admin-form-img"/>
                            </label>
                            <input type="file" name="thumbnail" id="photo" onchange="readURL(this);"
                                   class="d-none" value="{{ $response['blog']->thumbnail}}">
                        </div>
                    </div>
                </div>
                <div class="form-row mt-3">
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-xs-12 mb-4">
                        ...
                        <input type="text" name="title" id="title_id" value="{{ $response['blog']->title}}" class="admin-input form-control" placeholder="عنوان مطلب">
                    </div>
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-xs-12 mx-auto mb-4">
                        ...
                        <select id="category_id" name="category" class="form-control admin-input">
                            @foreach($response['parent'] as $parent)
                                <option value="{{$parent->id}}" class="admin-input" {{ $response['blog']->parent==$parent->id ? 'selected' : ''}}>{{$parent->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-row mt-3">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 mx-auto mb-4">
                        ...
                        <textarea class="form-control admin-input p-2 content" id="description_id" name="description" placeholder="محتوا">{{ $response['blog']->content}}</textarea>
                    </div>
                </div>
